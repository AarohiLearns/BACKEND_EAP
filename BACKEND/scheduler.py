    ...
    while True:
        ...
        for email in emails:
            ...
            if current_datetime >= scheduled_datetime:
                try:
                    # ---- CORE FIX ----
                    # The "attachments" list (manual, user-picked
                    # files) and attach_document's automatic pick
                    # are two independent mechanisms that were
                    # colliding: if the user already provided
                    # manual attachments via the attachments list,
                    # attach_document should NOT also fetch an
                    # unrelated automatic file -- that caused an
                    # extra, unwanted file being attached (and,
                    # on later recurring occurrences, an already-
                    # archived manual file being silently dropped,
                    # producing an empty email).
                    #
                    # attach_document's automatic-fetch behavior is
                    # now reserved for the case where NO manual
                    # attachments were provided at all (either via
                    # the attachments list, or via attachment_path
                    # from a prior manual-first-then-auto-recurring
                    # chain).
                    ...
                    for manual_attachment in resolved_attachments:
                        ...
                        # Do not archive the same file twice if it is also
                        # the automatic attachment.
                        if (
                            actual_attachment_path
                            and manual_path.resolve()
                            == Path(
                                actual_attachment_path
                            ).resolve()
                        ):
                    ...
                    if attach_document and actual_attachment_path:
                        ...
                        archive_file(
                            Path(actual_attachment_path)
                        )
                        print(
                            "Automatic attachment archived:",
                            actual_attachment_path,
                        )
